...
public class FixedMap<K, V> implements Map<K, V>, Serializable {
	...
	public FixedMap(final int maxEntries) {
	 ...
	 * @param maxEntries The maximum number of entries this map can hold
	 */
	public FixedMap(final Map<? extends K, ? extends V> map, final int maxEntries) {
		this(maxEntries);
		putAll(map);
	}
	...
	public void putAll(Map<? extends K, ? extends V> map) {
		for (final Iterator<? extends Entry<? extends K, ? extends V>> iterator = map.entrySet().iterator(); iterator
				.hasNext();) {
	...
	/**
	 * Finds the index of the next null key. If no null key can be found, the map is
	 * full and -1 will be returned.
	 * 
	 * @param start Index to start at
	...
	/**
	 * Finds a key by starting at lastSearchIndex and searching from there. If the
	 * key is found, lastSearchIndex is advanced so the next key search can find the
	 * next key in the array, which is the most likely to be retrieved.
	 * 
